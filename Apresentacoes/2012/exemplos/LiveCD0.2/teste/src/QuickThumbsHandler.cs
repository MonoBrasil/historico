...
public class QuickThumbsHandler : IHttpHandler {
	...
	private QuickThumbs quickThumbs;
	...
	//		string web_themeDir = QuickThumbs.CombinePath (context.Request.ApplicationPath, "bin/templates/") + GetSetting("template");

			string web_cssFile = QuickThumbs.CombinePath (context.Request.ApplicationPath,  "bin/templates/") + GetSetting("template") + "/" + GetSetting("theme") + ".css";
			...
			if (context.Request.QueryString["GetSpecialImage"] != null) {
				...
				foreach (FileInfo file in d.GetFiles()) {
					if (Array.IndexOf(validExtensions,file.Extension.ToLower()) > -1) {
						...
						//string thumbFile = QuickThumbs.CombinePath ("/bin/data/thumbs", HttpUtility.UrlDecode (context.Request.Path.Substring(context.Request.ApplicationPath.Length)) + file.Name + "-thumb.jpg");
						string thumbFile = QuickThumbs.CombinePath ("/bin/data/thumbs", relativePath + file.Name + "-thumb.jpg");
						...
						if (File.Exists(local_thumbFile) == false || file.LastWriteTime > new FileInfo(local_thumbFile).LastWriteTime) {
							...
							string localFile = context.Server.MapPath (QuickThumbs.CombinePath (HttpUtility.UrlDecode (context.Request.Path), file.Name));
		...
		//				tpl.setField ("PIC_THUMB_URL", context.Request.Url.ToString() + file.Name + "?Size=thumb"); //context.Request.ApplicationPath + thumbFile);
				...
				if (File.Exists(filePath)) {
					if (context.Request.Url.Query.IndexOf("ViewImage") > -1) {
						...
						string infoFile = context.Server.MapPath(QuickThumbs.CombinePath ("/bin/data/info", relativePath + ".txt"));

						ImageInfo info = new ImageInfo (filePath, infoFile);

						tpl = new Template (context.Server.MapPath( QuickThumbs.CombinePath ("/bin/templates",  GetSetting("template") + "/image.tpl")));
						
						
						tpl.setField ("PAGE_TITLE", fileName);
						tpl.setField ("PAGE_CSS", QuickThumbs.CombinePath (context.Request.ApplicationPath,  "bin/templates/") + GetSetting("template") + "/" + GetSetting("theme") + ".css");
						...
						// string smallFile = QuickThumbs.CombinePath (smallDir, relativePath +  "-" + imageWidth + ".jpg");

						string localSmallFile = context.Server.MapPath (QuickThumbs.CombinePath (smallDir, relativePath + "-" + imageWidth + ".jpg"));
						...
						while (i.MoveNext()) {
							...
							if (Array.IndexOf(tagsToShow,tagName.ToLower())  > -1) {
								Tag tag = (Tag)Enum.Parse(typeof(Tag),tagName,true);
						...
						if (context.Request.QueryString["Size"] != null) {
							...
							if (context.Request.QueryString ["Size"] == "thumb") {
								...
								fileName = QuickThumbs.CombinePath ("/bin/data/thumbs", fileName + "-thumb.jpg");
								//fileName = QuickThumbs.CombinePath ("/bin/data/thumbs", HttpUtility.UrlDecode (context.Request.Path) + "-thumb.jpg");
			...
			tpl.setField("QUICKTHUMBS_VERSIONSTRING", "QuickThumbs 0.23");
