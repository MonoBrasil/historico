...
%{
	private bool short_tags;
	private bool asp_tags;
	...
	private void yy_push_state(int new_state) {
		zzLexicalStateStack.Push(zzLexicalState);
		yybegin(new_state);
	}
	private void yy_pop_state() {
		int old_state = (int)zzLexicalStateStack.Pop();
		yybegin(old_state);
	}
	private int yy_top_state() {
		return (int)zzLexicalStateStack.Peek();
	}
	
	private bool is_oct(char c) {
		return c >= '0' && c <= '8';
	}
...
<YYINITIAL>"<%="|"<?=" {
	if ((yycharat(1)=='%' && asp_tags) || (yycharat(1)=='?' && short_tags)) {
		yybegin(ST_IN_SCRIPTING);
...
<ST_ONE_LINE_COMMENT>"?>"|"%>" {
    if (asp_tags || yycharat(yylength()-2) != '%') { /* asp comment? */
